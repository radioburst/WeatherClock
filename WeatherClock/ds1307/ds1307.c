 ...
 * get date
 */
void ds1307_getdate(uint8_t *year, uint8_t *month, uint8_t *day, uint8_t *hour, uint8_t *minute, uint8_t *second, uint8_t *dayofweek) {
	i2c_start_wait(DS1307_ADDR | I2C_WRITE);
	i2c_write(0x00);//stop oscillator
	i2c_stop();
	...
	*year = ds1307_bcd2dec(i2c_readNak());	
	i2c_stop();
	*dayofweek = ds1307_getdayofweek(*year, *month, *day);
}
